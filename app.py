...
@st.cache_data
def luo_vakio_esikatselupohja(preview_w):
    ...
    for mm in range(0, 1001, 10): 
        ...
        if x <= m_p + a_w:
            ...
            else: draw.line([x, m_p - 6, x, m_p], fill=(150,150,150,255), width=1)
    ...
    for mm in range(0, 561, 10):
        ...
        if y <= m_p + a_h:
            ...
            else: draw.line([m_p - 6, y, m_p, y], fill=(150,150,150,255), width=1)
...
with col1:
    with st.expander("📖 Käyttöohjeet", expanded=False):
        st.markdown("1. Lataa logo\n2. Valitse listasta\n3. Poista tausta/Vaihda väri\n4. Aseta mm-koko ja sijoita")
    ...
    if uusi:
        if uusi.name not in st.session_state.kuvat:
            try:
                ...
                else: img = Image.open(uusi).convert("RGBA")
                ...
                st.rerun()
            except Exception as e: st.error(f"Latausvirhe: {e}")
    ...
    if st.session_state.kuvat:
        ...
        for n in list(st.session_state.kuvat.keys()):
            with st.container(border=True):
                ...
                if cl2.button("VALITSE", key=f"sel_{n}", type="primary" if st.session_state.valittu == n else "secondary", use_container_width=True):
                    st.session_state.valittu = n; st.rerun()
                if cl2.button("🗑️ Poista", key=f"del_{n}", use_container_width=True):
    ...
    if st.session_state.valittu:
        ...
        dpi = dpi_t[0] if isinstance(dpi_t, tuple) else (dpi_t if dpi_t else 300)
        ...
        if st.button("🚀 SIJOITA ARKKIIN", type="primary", use_container_width=True):
            st.session_state.historia.append({"arkki": st.session_state.arkki.copy(), "occ": st.session_state.occ.copy()})
            if len(st.session_state.historia) > 5: st.session_state.historia.pop(0)

            w_px, h_px = int(l_mm * DPI_VAKIO), int(l_mm * suhde * DPI_VAKIO)
        ...
        if st.session_state.historia:
            if st.button("↩️ PERUUTA VIIMEISIN SIJOITUS", use_container_width=True, key="peruuta_btn"):
                viim = st.session_state.historia.pop()
                st.session_state.arkki, st.session_state.occ = viim["arkki"], viim["occ"]
                st.rerun()
        else: st.button("↩️ PERUUTA", disabled=True, use_container_width=True, key="peruuta_dis")

with col2:
    st.header("2. Esikatselu")
    p_img, m_p, sk_l = luo_vakio_esikatselupohja(PREVIEW_W)
    pohja = p_img.copy()
    a_w, a_h = int(ARKKI_L * sk_l), int(ARKKI_K * sk_l)
    arkki_pieni = st.session_state.arkki.resize((a_w, a_h), Image.Resampling.NEAREST)
    pohja.paste(arkki_pieni, (m_p, m_p), arkki_pieni)
    st.image(pohja, use_container_width=False)
    
    c_down, c_clr = st.columns(2)
    buf = io.BytesIO()
    st.session_state.arkki.save(buf, format="PNG")
    c_down.download_button("📥 Lataa PNG", buf.getvalue(), "arkki.png", use_container_width=True)
